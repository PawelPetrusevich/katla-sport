...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number){
    var hiveSection = this.hiveSections.find(x=>x.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId,true).subscribe(x => hiveSection.isDeleted = true);
  }

  unDelete(hiveSectionId: number){
    var hiveSection = this.hiveSections.find(x=>x.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId,false).subscribe(x=>hiveSection.isDeleted = false);
  }
}
